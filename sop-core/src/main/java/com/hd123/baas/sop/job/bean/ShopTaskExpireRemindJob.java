...
@Slf4j
@Component
public class ShopTaskExpireRemindJob implements Job {
  ...
  public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
    ...
    try {
      ...
      log.error("执行门店巡检任务临期提醒job异常：" + JsonUtil.objectToJson(e));
    }
  }

  public void expireRemind(String tenant, Date remindTime) throws BaasException {
    log.info("开始执行租户<{}>任务提醒，提醒时间：{}", tenant, remindTime);
    List<ShopTask> shopTasks = shopTaskService.listByRemindTime(tenant, remindTime);
    ...
    if (CollectionUtils.isNotEmpty(shopTasks)) {
      for (ShopTask shopTask : shopTasks) {
        ...
        logs = logs != null
            ? logs.stream().filter(s -> s.getState().equals(ShopTaskState.UNFINISHED.name())).collect(Collectors.toList())
            : null;
  ...
  private Message buildCheckMessage(String tenant, ShopTask shopTask, String userId, String checkTaskPage) {
    ...
    content.put(MessageContentKey.TEXT, "您有巡检计划尚未完成:" + shopTask.getName());
